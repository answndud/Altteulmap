...
최근에는 고물가 상황에서 저렴한 식당과 생활 서비스 정보를 지도 기반으로 찾고 제보할 수 있는 서비스인 알뜰맵을 개발했습니다. 단순히 장소 목록을 보여주는 데서 끝내지 않고, 공공 데이터 import, 지도 탐색, 가격 항목, 사용자 제보, 신고, 북마크, 관리자 승인, AI 1차 검수, 운영 배포와 smoke 검증까지 하나의 MVP로 닫았습니다.
...
또한 제품 내부에서도 AI를 운영자 판단 보조로 사용했습니다. 장소 등록, 가격 제보, 신고 큐에는 `AI 1차 검수` 패널을 두고, suggested action, confidence, summary, checks, flags를 보여줍니다. 중요한 점은 AI가 최종 결정을 하지 않는다는 것입니다. 운영자는 AI 제안을 참고하되 승인/반려/상태 변경을 직접 확정하고, 시스템은 그 결과를 DB와 audit 흐름에 남기는 구조입니다.
...
제품 흐름은 다음과 같습니다. 사용자는 지도에서 저렴한 장소를 찾고, 가격과 상세 정보를 확인하고, 비회원 상태에서도 장소/가격/댓글/신고를 제출할 수 있습니다. 운영자는 관리자 화면에서 제출 내용을 확인하고, AI 1차 검수 제안을 참고해 승인/반려를 확정합니다. 승인된 데이터는 다시 지도 검색과 상세 화면에 반영됩니다.
...
## 짧은 제출 버전
저는 실제 문제를 제품 구조, 코드, 운영 검증까지 연결하는 풀스택 개발자입니다. 최근에는 저렴한 식당과 생활 서비스 정보를 지도 기반으로 탐색하고 제보할 수 있는 알뜰맵을 개발했습니다. 이 프로젝트는 Vite React SPA, Cloudflare Worker API, Supabase PostgreSQL, NAVER Maps, 관리자 화면, AI 1차 검수, Playwright E2E, local/remote smoke, CI/deploy check까지 포함한 운영 가능한 MVP입니다.
