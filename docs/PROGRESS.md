...
Next.js 기능 parity 회귀 복구 구현은 1차 완료됐다. Review Finding 1~4의 자동화 검증 기준은 통과했고, 가격 필터 spec과 운영 지도 key 문제도 정리했다. 운영 배포와 remote smoke까지 통과했으며, 남은 작업은 실제 Kakao/Naver OAuth live callback과 실기기 수동 QA다.
...
| Public API | `/api/places/map`, `/api/places/:id`, price report, comments, reaction, submission, reports, bookmarks가 Worker route로 연결되어 E2E 통과했다. 가격 필터는 MVP 미노출로 확정했다. | 복구 완료 | 없음 |
