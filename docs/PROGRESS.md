...
- P3 큰 파일 분리 리팩터링의 다음 slice로 `src/client/routes/MapRoute.tsx`의 search/header 영역 또는 `src/features/map/naver-map-panel.tsx`의 marker/cluster 영역을 추가 분리한다.
