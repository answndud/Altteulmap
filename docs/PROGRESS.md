...
- 상태:
  ...
  - smoke/deploy check 스크립트의 URL truthiness, https, localhost, canonical 비교 helper를 공통 모듈로 분리했다.
- 현재 범위:
...
- 최근 검증:
  ...
  - GoodPrice helper 분리 후 `npm run typecheck`
    - 통과
  - admin entrypoint re-export 전환 후 `npm run lint`
    - 통과
  - admin entrypoint re-export 전환 후 `npm run typecheck`
    - 통과
  ...
  - 폼 피드백 컴포넌트 분리 후 `npm run typecheck`
    - 통과
  - 운영 URL helper 분리 후 `npm run lint`
    - 통과
  - 운영 URL helper 분리 후 `npm run typecheck`
    - 통과
  - 운영 URL helper 분리 후 `npm run deploy:check:admin`
    - 통과
  - 운영 URL helper 분리 후 `npm run deploy:check:public`
    - 통과
