...
## `071` public write 댓글 route 분리

- 배경:
  - `docs/refactoring-large-files.md` 기준 다음 hotspot은 `src/worker/routes/public-write.ts`였다.
  - 해당 파일은 가격 제보, 댓글 등록/삭제, 반응, 장소 등록, 신고 route를 한 module에서 처리하고 있었다.
  - API route refactor이므로 기존 response shape, status code, rate limit, Turnstile, DB/mock fallback 동작을 유지하는 얇은 slice로 진행했다.
- 변경 내용:
  - `src/worker/routes/public-write-comments.ts`를 추가했다.
  - `POST /api/places/:id/comments`와 `DELETE /api/places/:id/comments/:commentId` 등록을 새 module로 이동했다.
  - 기존 `registerPublicWriteRoutes`에서는 같은 위치에서 `registerPublicWriteCommentRoutes(app, dependencies)`를 호출해 route 등록 순서를 유지했다.
  - 댓글 등록의 rate limit, zod validation, Turnstile 검증, DB write, mock fallback response를 그대로 유지했다.
  - 댓글 삭제의 DB/mock 권한 검사와 status mapping을 그대로 유지했다.
...
- 검증:
  ...
  - `git diff --check`
    - 통과.
- 결과:
  - `src/worker/routes/public-write.ts`는 720 lines에서 500 lines로 줄었다.
...
## `073` public write place submission route 분리

- 배경:
  - reaction route 분리 후 `src/worker/routes/public-write.ts`에는 가격 제보, 장소 등록, 신고 route가 남아 있었다.
  - `POST /api/places`는 schema validation, rate limit, Turnstile, DB/mock fallback, preview response를 담당해 독립 route module로 분리하기 적합했다.
  - API route refactor이므로 기존 response shape, status code, rate limit, Turnstile, DB/mock fallback 동작을 유지했다.
- 변경 내용:
  - `src/worker/routes/public-write-submissions.ts`를 추가했다.
  - `POST /api/places` 등록을 새 module로 이동했다.
  - 기존 `registerPublicWriteRoutes`에서는 같은 위치에서 `registerPublicWriteSubmissionRoutes(app, dependencies)`를 호출해 route 등록 순서를 유지했다.
  - 장소 등록 입력 validation 실패, rate limit 실패, Turnstile 실패, DB write result, mock preview response shape를 그대로 유지했다.
  - `docs/refactoring-large-files.md`의 public write hotspot line count와 Worker Slice 4 상태를 갱신했다.
- 코드/문서:
  - `src/worker/routes/public-write.ts`
...
- 검증:
  ...
  - `git diff --check`
    - 통과.
- 결과:
  - `src/worker/routes/public-write.ts`는 386 lines에서 282 lines로 줄었다.
  - place submission route path, response shape, rate limit, Turnstile, DB/mock fallback 동작은 변경하지 않았다.
  - 다음 구조 개선 후보는 public write의 report submission route 또는 price report route 분리다.
  - active 문서는 `현재 active 작업 없음` 상태로 정리했다.

## `074` public write report submission route 분리

- 배경:
  - place submission route 분리 후 `src/worker/routes/public-write.ts`에는 가격 제보와 신고 route가 남아 있었다.
  - `POST /api/reports`는 schema validation, rate limit, Turnstile, DB/mock fallback, preview response를 담당해 독립 route module로 분리하기 적합했다.
  - API route refactor이므로 기존 response shape, status code, rate limit, Turnstile, DB/mock fallback 동작을 유지했다.
- 변경 내용:
  - `src/worker/routes/public-write-reports.ts`를 추가했다.
  - `POST /api/reports` 등록을 새 module로 이동했다.
  - 기존 `registerPublicWriteRoutes`에서는 같은 위치에서 `registerPublicWriteReportRoutes(app, dependencies)`를 호출해 route 등록 순서를 유지했다.
  - 신고 입력 validation 실패, rate limit 실패, Turnstile 실패, DB write result, mock preview response shape를 그대로 유지했다.
  - `docs/refactoring-large-files.md`의 public write hotspot line count와 Worker Slice 4 상태를 갱신했다.
- 코드/문서:
  - `src/worker/routes/public-write.ts`
  - `src/worker/routes/public-write-reports.ts`
  - `docs/refactoring-large-files.md`
  - `docs/PLAN.md`
  - `docs/PROGRESS.md`
  - `docs/COMPLETED.md`
- 검증:
  ...
  - `git diff --check`
    - 통과.
- 결과:
  - `src/worker/routes/public-write.ts`는 282 lines에서 173 lines로 줄었다.
  - report submission route path, response shape, rate limit, Turnstile, DB/mock fallback 동작은 변경하지 않았다.
  - 다음 구조 개선 후보는 public write의 마지막 price report route 분리다.
  - active 문서는 `현재 active 작업 없음` 상태로 정리했다.

## `075` public write price report route 분리

- 배경:
  - report submission route 분리 후 `src/worker/routes/public-write.ts`에는 마지막으로 가격 제보 route만 직접 남아 있었다.
  - `POST /api/places/:id/prices`는 schema validation, rate limit, Turnstile, DB/mock fallback, mock place lookup을 담당해 독립 route module로 분리하기 적합했다.
  - API route refactor이므로 기존 response shape, status code, rate limit, Turnstile, DB/mock fallback 동작을 유지했다.
- 변경 내용:
  - `src/worker/routes/public-write-price-reports.ts`를 추가했다.
  - `POST /api/places/:id/prices` 등록을 새 module로 이동했다.
  - 기존 `registerPublicWriteRoutes`에서는 같은 위치에서 `registerPublicWritePriceReportRoutes(app, dependencies)`를 호출해 route 등록 순서를 유지했다.
  - 가격 제보 입력 validation 실패, rate limit 실패, Turnstile 실패, DB write result와 404 mapping, mock place not found, mock preview item response shape를 그대로 유지했다.
  - `docs/refactoring-large-files.md`에서 public write route hub 상태와 Worker Slice 4 완료 상태를 갱신했다.
- 코드/문서:
  - `src/worker/routes/public-write.ts`
  - `src/worker/routes/public-write-price-reports.ts`
  - `docs/refactoring-large-files.md`
  - `docs/PLAN.md`
  - `docs/PROGRESS.md`
  - `docs/COMPLETED.md`
- 검증:
  ...
  - `git diff --check`
    - 통과.
- 결과:
  - `src/worker/routes/public-write.ts`는 173 lines에서 30 lines로 줄어 route hub 역할만 남았다.
  - price report route path, response shape, rate limit, Turnstile, DB/mock fallback 동작은 변경하지 않았다.
...
## `077` places read map query 분리

- 배경:
  - mock fallback 분리 후 `src/worker/places-read-repository.ts`에는 map list DB query와 place detail DB query가 함께 남아 있었다.
  - map list query는 where/order/count, preview cache, marker mode, bounds 계산을 함께 담당해 별도 module로 분리하기 적합했다.
  - 이번 slice는 `/api/places/map`의 response shape와 cache/marker 계산 동작을 유지하면서 DB map query만 분리했다.
- 변경 내용:
  - `src/worker/places-read-map-repository.ts`를 추가했다.
  - `listDatabaseMapPlaces`와 map query helper를 새 module로 이동했다.
  - viewport bounds cache hit/miss/bypass, marker limit, cluster/place marker mode, list cap, count fallback 계산을 그대로 유지했다.
  - `src/worker/places-read-repository.ts`는 public entry point와 place detail DB query 중심으로 줄였다.
  - `docs/refactoring-large-files.md`의 `places-read-repository` line count와 다음 작업 순서를 갱신했다.
- 코드/문서:
  - `src/worker/places-read-repository.ts`
  - `src/worker/places-read-map-repository.ts`
  - `docs/refactoring-large-files.md`
  - `docs/PLAN.md`
  - `docs/PROGRESS.md`
  - `docs/COMPLETED.md`
- 검증:
  ...
  - `git diff --check`
    - 통과.
- 결과:
  - `src/worker/places-read-repository.ts`는 502 lines에서 256 lines로 줄었다.
...
## `079` admin report route 분리

- 배경:
  - `places-read-repository` 분리 완료 후 다음 구조 개선 후보는 `src/worker/routes/admin.ts`였다.
  - admin route module에는 places, prices, reports domain이 함께 등록되어 있었다.
  - 이번 slice는 독립성이 높은 신고 검토 route와 공통 admin support만 먼저 분리했다.
- 변경 내용:
  - `src/worker/routes/admin-support.ts`를 추가했다.
  - admin binding/dependency type과 `requireAdminSession` guard를 support module로 이동했다.
  - `src/worker/routes/admin-reports.ts`를 추가했다.
  - `/api/admin/reports`, `/api/admin/reports/:id` route 등록을 새 module로 이동했다.
...
- 코드/문서:
  ...
  - `src/worker/routes/admin-support.ts`
  - `docs/refactoring-large-files.md`
  - `docs/PLAN.md`
  - `docs/PROGRESS.md`
  - `docs/COMPLETED.md`
- 검증:
  ...
  - `git diff --check`
    - 통과.
- 결과:
  - `src/worker/routes/admin.ts`는 643 lines에서 471 lines로 줄었다.
...
## `081` admin place route 분리

- 배경:
  - price route 분리 후 `src/worker/routes/admin.ts`에는 마지막으로 place domain route만 직접 남아 있었다.
  - place route는 pending place list, pending place detail lookup, place moderation을 담당해 독립 module로 분리하기 적합했다.
  - 이번 slice는 admin route hub를 완성하면서 기존 status/response/mock fallback을 유지했다.
- 변경 내용:
  - `src/worker/routes/admin-places.ts`를 추가했다.
  - `/api/admin/places`, `/api/admin/places/:id` GET/PATCH route 등록을 새 module로 이동했다.
  - 기존 401/403/400/404/500 status, no-store headers, DB/mock fallback, response shape를 유지했다.
  - `src/worker/routes/admin.ts`는 place/price/report route module 등록만 담당하는 hub로 줄였다.
  - `docs/refactoring-large-files.md`의 admin route line count와 Worker Slice 5 완료 상태를 갱신했다.
- 코드/문서:
  - `src/worker/routes/admin.ts`
...
- 검증:
  ...
  - `git diff --check`
    - 통과.
- 결과:
  - `src/worker/routes/admin.ts`는 202 lines에서 24 lines로 줄어 route hub 역할만 남았다.
  - admin place API path, response shape, authorization semantics, mock/DB fallback 동작은 변경하지 않았다.
...
## `087` places write price report repository 분리

- 배경:
  - reaction repository 분리 후 `src/worker/places-write-repository.ts`에는 place submission과 price report write가 남아 있었다.
  - price report 로직은 active place lookup, matched price item lookup, pending report insert, response mapping으로 구성되어 place submission transaction과 독립적으로 분리 가능했다.
  - 이번 slice는 price report API path와 response shape를 바꾸지 않는 동작 보존형 분리로 제한했다.
- 변경 내용:
  - `src/worker/places-write-price-reports-repository.ts`를 추가했다.
  - `createDatabasePlacePriceReport`를 price report repository module로 이동했다.
  - matched price item lookup과 pending price report insert 로직을 새 module로 이동했다.
  - `src/worker/routes/public-write-price-reports.ts`는 새 price report repository를 직접 import하게 변경했다.
  - `src/worker/places-write-repository.ts`에는 place submission repository logic만 남겼다.
  - `docs/refactoring-large-files.md`의 places write hotspot line count와 분리 상태를 갱신했다.
- 코드/문서:
  - `src/worker/places-write-repository.ts`
  - `src/worker/places-write-price-reports-repository.ts`
  - `src/worker/routes/public-write-price-reports.ts`
  - `docs/refactoring-large-files.md`
  - `docs/PLAN.md`
  - `docs/PROGRESS.md`
...
- 검증:
  ...
  - `git diff --check`
    - 통과.
- 결과:
  - `src/worker/places-write-repository.ts`는 240 lines에서 170 lines로 줄었다.
...
## `104` Drizzle operational table group 분리

- 배경:
  - auth/user table group 분리 후 `src/db/schema.ts`에는 telemetry와 public write rate limit처럼 운영성 테이블도 남아 있었다.
  - `visitActivities`는 `users.id`만 참조하고 `publicWriteRateLimits`는 FK가 없어 별도 module로 이동해도 순환 의존을 만들지 않는다.
  - 이번 slice는 DB 구조를 바꾸지 않고 operational table 정의 위치와 re-export 경계만 바꾸는 동작 보존형 리팩터링으로 제한했다.
- 변경 내용:
  - `src/db/schema-operational.ts`를 추가했다.
  - `visitActivities`, `publicWriteRateLimits` table 정의를 새 module로 이동했다.
  - `src/db/schema.ts`는 operational table group을 다시 export해 기존 `@/db/schema` import contract를 유지했다.
  - `visit_activity`의 actor/group/bucket unique index, date/source/user/visitor indexes, `users.id` FK를 유지했다.
  - `public_write_rate_limits`의 composite primary key, expires index, actor updated index를 유지했다.
  - `docs/refactoring-large-files.md`의 schema hotspot line count와 분리 상태를 갱신했다.
- 코드/문서:
  - `src/db/schema.ts`
...
- 검증:
  ...
  - `npm run test:e2e:full -- tests/e2e/comments.spec.ts`
    - 통과.
    - runner 특성상 DB push/seed/build 후 desktop smoke 10개, desktop full 7개, mobile 3개가 실행되어 모두 통과했다.
    - comments rate-limit flow와 `db:push` no changes detected가 확인됐다.
    - 로컬 Node `v20.13.1`에서 Vite의 Node `20.19+` 권장 경고가 출력됐지만 build/test는 통과했다.
- 결과:
  - `src/db/schema.ts`는 464 lines에서 392 lines로 줄었다.
