...
- MVP에서는 가격 범위 필터 UI와 `minPrice`/`maxPrice` API contract를 제공하지 않는다.
- 가격 범위 필터는 카테고리별 대표 가격 기준과 데이터 품질이 충분해진 뒤 후속 기능으로 추가한다.
...
### 7.4 데스크톱 웹 UI 원칙
- 지도와 장소 목록을 한 화면에서 함께 탐색
- 필터와 검색 상태를 명확히 유지
- 상세 정보는 현재 탐색 맥락을 잃지 않는 패널로 제공
- 주요 조작은 키보드와 포인터로 접근 가능
