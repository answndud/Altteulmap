...
- 디자인/지도 품질 복구
  ...
  - 지도 pan 중 작은 bounds 변화로 숫자 클러스터 위치가 계속 흔들리지 않도록 서버 cluster grid는 viewport origin이 아니라 고정 world grid 기준으로 계산한다.
  - viewport API 요청은 raw map bounds가 아니라 snap된 bounds/zoom으로 만들어 작은 이동이 매번 cache miss와 `검색 중` 상태를 만들지 않게 한다.
  - 자동 viewport 갱신 중에는 사용자가 누르는 refresh 버튼을 긴 시간 `검색 중`으로 묶어두지 않는다.
  ...
  - optimistic split 전환 시 기존 Naver cluster marker를 즉시 제거해 숫자 클러스터와 임시 place marker가 동시에 보이지 않게 한다.
