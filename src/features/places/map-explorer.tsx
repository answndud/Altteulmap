...
function ListStatusSummary({
  ...
  const outerSpacingClassName = compact ? "mb-2.5 grid gap-2" : "mb-3 grid gap-2";
  const panelPaddingClassName = compact ? "px-3 py-3" : "px-4 py-3";
  const badgeClassName = compact
    ? "altteulmap-badge whitespace-nowrap px-2.5 py-1 text-[11px] font-medium"
    : "altteulmap-badge whitespace-nowrap px-3 py-1.5 text-xs font-medium";
  ...
  return (
    <div className={outerSpacingClassName}>
      {fetchError ? (
        <div
          className={`rounded-[1rem] border border-amber-200 bg-amber-50 text-sm text-amber-800 ${panelPaddingClassName}`}
        >
          {fetchError}
        </div>
      ) : null}
...
function PlaceList({
  ...
  const listGapClassName = compact ? "grid gap-2.5" : "grid gap-3";
  ...
  return (
    <div className={listGapClassName} data-testid={listTestId}>
      {places.map((place) => {
        ...
        const titleSizeClassName = compact ? "text-[0.98rem]" : "text-[1.02rem]";
        const sharePayload = shouldShowSecondaryActions
          ? createPlaceSharePayload(place, "list")
          : null;
        const priceLabel = place.representativePriceLabel || "대표 가격";
        ...
        return (
          <article
            ...
            className={`rounded-[1rem] border text-left transition focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 ${itemPaddingClassName} ${
              isActive
                ? "border-[var(--altteul-accent-border)] bg-[var(--altteul-accent-soft)]/70 shadow-sm"
                : "border-stone-200 bg-white hover:border-stone-300 hover:bg-[var(--altteul-surface-fill-hover)]"
            }`}
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p
                  className={`altteulmap-price-number mt-1 ${compact ? "text-xl" : "text-2xl"}`}
                >
            ...
            <div className="mt-3 min-w-0">
              <h3 className={`truncate font-semibold text-stone-950 ${titleSizeClassName}`}>
                {place.name}
              </h3>
              <p className="mt-1 truncate text-xs text-stone-600">
                {[category?.name ?? "기타", place.district].filter(Boolean).join(" · ")}
              </p>
            </div>
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <span
                className="altteulmap-badge whitespace-nowrap px-2.5 py-1 text-[11px] font-medium"
        ...
        <section className="hidden max-h-[44rem] flex-col overflow-hidden rounded-[1.125rem] border border-stone-200 bg-white shadow-sm xl:flex">
          <div className="flex items-center justify-between border-b border-stone-200 px-4 py-4">
            ...
            <div className="flex items-center gap-2">
              <p className="altteulmap-badge whitespace-nowrap px-3 py-1 text-xs font-medium">
                {totalPlaceCount}곳
              </p>
            </div>
          </div>
      ...
      {isMobileListOpen ? (
        <div className="fixed inset-0 z-[85] xl:hidden">
          ...
          <section
            ...
            className="altteulmap-mobile-sheet altteulmap-mobile-sheet-list absolute flex flex-col overflow-hidden rounded-[1.125rem] border border-stone-200 bg-white shadow-2xl"
          >
            <div className="sticky top-0 z-10 border-b border-stone-200 bg-white/96 px-4 pb-2.5 pt-2 backdrop-blur">
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="altteulmap-section-kicker text-[10px]">목록</p>
                ...
                <div className="flex shrink-0 items-center gap-2">
                  ...
                  >
                    {mobileListSheetMode === "peek" ? "더 보기" : "줄이기"}
                  </button>
                  <button
                    type="button"
                    onClick={closeMobileList}
